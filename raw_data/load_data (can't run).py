...
def _parse_args(argv=None):
    ...
    parser.add_argument(
        "--file_pattern",
        required=True,
        help="File pattern for amazon qa files on local files.",
    )
    parser.add_argument(
        "--output_dir",
        required=True,
        help="Output directory to write the dataset on local files.",
    )
    parser.add_argument(
        "--dataset_format",
...
def run(argv=None):
    ...
    # Create a partial function with _should_skip pre-bound:
    pre_bound_should_skip = partial(_should_skip, min_words=args.min_words, max_words=args.max_words)

    qa_tuples = json_objects | "create tuples" >> beam.FlatMap(
            _create_tuples, min_words=args.min_words, max_words=args.max_words)
